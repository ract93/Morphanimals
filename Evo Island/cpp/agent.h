...
struct Agent {
    ...
    // Decoded traits — mirrors genome indices 0-5.
    ...
    // speed=0 preserves plant-like behaviour as the evolutionary baseline.
    static constexpr float ANCESTOR[6] = {20.0f, 10.0f, 5.0f, 5.0f, 3.0f, 0.0f};
    ...
    static Agent create_live(const float genome[6]);
    static Agent create_live_default();
    static void  mutate_genome(const float in[6], float out[6],
    ...
    static void  genome_to_color(const float genome[6], float color[3]);
};
